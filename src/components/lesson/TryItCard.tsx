    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className={`inline-flex h-9 w-9 items-center justify-center rounded-full text-sm font-black text-white ${isComplete ? "bg-[#00AFB9]" : "bg-[#073B5A]"}`}>
            {isComplete ? "✓" : "3"}
          </span>
          <div>
            <h3 className="text-lg font-black text-[#073B5A]">Try It</h3>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#0081A7]">Guided Step</p>
          </div>
        </div>
        <p className="text-xs font-bold text-[#073B5A]/65">10 min</p>
      </div>

      <p className="mt-4 line-clamp-2 text-sm font-semibold leading-6 text-[#073B5A]">{preview.tip}</p>

      <div className="mt-4 rounded-2xl border border-[#00AFB9]/15 bg-[#E9F7F8] px-3 py-3">
        <p className="line-clamp-2 text-sm font-black leading-5 text-[#073B5A]">{preview.question}</p>
        <div className="mt-2 flex flex-wrap items-center gap-1.5 text-lg">
          {Array.from({ length: groupCount }).map((_, index) => (
            <span key={index} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white shadow-inner">
              {preview.visualEmoji}
            </span>
          ))}
        </div>
