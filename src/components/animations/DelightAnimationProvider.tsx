...
type DustParticleSeed = Omit<DustParticle, "id" | "toX" | "toY" | "trailId">;
...
function FlyingDustParticle({ particle, onDone }: DustParticleProps) {
  ...
  const midY = Math.min(particle.fromY, particle.toY) - 95 + particle.curveOffsetY;
    ...
    <div className="pointer-events-none fixed inset-0 z-[999999] overflow-visible">
      <AnimatePresence>
        {leadParticles.map((particle) => (
          <FlyingLeadParticle key={particle.id} particle={particle} onDone={removeLeadParticle} />
        ))}

        {dustParticles.map((particle) => (
          <FlyingDustParticle key={particle.id} particle={particle} onDone={removeDustParticle} />
        ))}
...
export function DelightAnimationProvider({ children }: DelightAnimationProviderProps) {
  ...
  const removeLeadParticle = useCallback((id: string) => {
    setLeadParticles((current) => current.filter((particle) => particle.id !== id));
  }, []);

  const removeDustParticle = useCallback((id: string) => {
    setDustParticles((current) => current.filter((particle) => particle.id !== id));
  }, []);

  const removeBurstParticle = useCallback((id: string) => {
    setBurstParticles((current) => current.filter((particle) => particle.id !== id));
  }, []);
  ...
  const triggerArrivalBurst = useCallback((x: number, y: number, trailId: SparkleTrailId) => {
