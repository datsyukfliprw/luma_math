...
import { getLessonExperience, type ResolvedLessonExperience } from "../data/lessonExperience";
...
function getLessonIdFromLesson(lesson: LearnLesson) {
  const { unit_number, week_number, day_number, lesson_type } = lesson as CurriculumLearnLesson;

  if (unit_number === undefined || week_number === undefined || day_number === undefined) {
    return undefined;
  }

  // Format: g{grade}-u{unit}-w{week}-l{lessonNumber}
  if (lesson_type === "evaluation") {
    return `g3-u${unit_number}-w${week_number}-eval`;
  }

  return `g3-u${unit_number}-w${week_number}-l${day_number}`;
}
...
function requireExperience(lesson: LearnLesson): ResolvedLessonExperience {
  const experience = getLessonExperienceForLearnLesson(lesson);

  if (!experience) {
    throw new Error(`No lesson experience found for lesson.`);
  }
...
export function getBuildRounds(lesson: LearnLesson): BuildRound[] {
  return (
    requireExperience(lesson).buildIt?.rounds.map((round) => ({
      groups: round.groups,
...
export function getSeeItClues(lesson: LearnLesson): SeeItClue[] {
  return (
    requireExperience(lesson).seeIt?.clues.map((clue) => ({
...
export function getSeeItExamples(lesson: LearnLesson) {
  return (
    requireExperience(lesson).seeIt?.examples.map((example, index) => {
...
export function getQuickCheckQuestions(lesson: LearnLesson): QuickCheckQuestion[] {
  return (
    requireExperience(lesson).quickCheck?.questions.map((question) => ({
