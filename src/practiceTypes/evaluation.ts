...
function getReviewTypes(options?: PracticeGenerationOptions): ReviewType[] {
  ...
  return lessonReviewTypes.filter(
    (type): type is ReviewType => type in reviewGenerators,
  )
}

export function generateEvaluationProblems(
  options?: PracticeGenerationOptions,
): PracticeProblem[] {
  const reviewTypes = getReviewTypes(options)
  const questionCount =
    options?.lesson?.quiz_question_count ??
    options?.lesson?.practice_block?.question_count ??
    9
  ...
  while (mixedProblems.length < questionCount && pools.length > 0) {
    ...
    const hasMoreProblems = pools.some(
      (pool) => pool.index < pool.problems.length,
    )
