...
function buildSeed(options: PracticeGenerationOptions | undefined, mode: string): string {
  ...
  const lessonId = options?.lesson?.lesson_id ?? "large_digit_value";
  return createPracticeSessionSeed(lessonId, "large_digit_value", mode);
}
...
function generateUniqueProblem(
  ...
  for (let attempt = 0; attempt < MAX_PROBLEM_GENERATION_ATTEMPTS; attempt += 1) {
    ...
    const problem = generateDigitValueProblem(rng, { numberRange: LARGE_NUMBER_RANGE });

    if (!usedKeys.has(problem.problemKey)) {
      return {
        id: `large-digit-value-${mode}-${index + 1}`,
  ...
  throw new Error(
    `Could not generate a unique large_digit_value problem within ${MAX_PROBLEM_GENERATION_ATTEMPTS} attempts`,
  );
}

export function generateLargeDigitValueProblems(
